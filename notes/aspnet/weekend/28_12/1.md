# **MVC fundamentals meet real-world distributed architecture**

Imagine you built a **beautiful shopping mall** 🏬.

* Customers (Browser) enter the mall
* They add items to a **shopping basket**
* The basket must **remember items** even if:

  * They go to another counter
  * The mall staff changes
  * Traffic is heavy

Now imagine this mall has **multiple gates (servers)**.

If the basket is kept **inside one gate’s cupboard (In-Memory Session)**:

* Customer enters Gate-1 → basket exists
* Next request goes to Gate-2 → basket is **lost** ❌

That’s exactly what happens when:

> Session state is stored **inside the ASP.NET Web Server**


## 🚨 The Core Problem

ASP.NET Core MVC is **stateless by design**
HTTP is **stateless**
Load balancers send requests to **different servers**

👉 **Session in memory will NOT work**


## ✅ Solution Principle (Golden Rule)

> **Session State must live OUTSIDE the Web Server**


## 🧩 Architectural Options (Real-World)

### 🥇 Option 1: **Distributed Cache (Recommended)**

Store session in a **centralized system**:

| Option                   | Use Case                      |
| ------------------------ | ----------------------------- |
| **Redis**                | Production, scalable, fastest |
| SQL Server               | Small / medium apps           |
| Distributed Memory Cache | Learning only                 |

### 🏗️ Architecture

```
Browser
   |
Load Balancer
   |
-------------------------
| Web Server A (MVC)   |
| Web Server B (MVC)   |
| Web Server C (MVC)   |
-------------------------
        |
        v
   Redis / SQL Server
   (Session Store)
```

- ✔ Any server can access the same session
- ✔ No session loss
- ✔ Horizontal scaling supported



## 🧪 ASP.NET Core Implementation (Redis Example)

### 1️⃣ Install Packages

```bash
dotnet add package Microsoft.Extensions.Caching.StackExchangeRedis
```

### 2️⃣ Configure `Program.cs`

```csharp
builder.Services.AddStackExchangeRedisCache(options =>
{
    options.Configuration = "localhost:6379";
    options.InstanceName = "ShoppingCartApp:";
});

builder.Services.AddSession(options =>
{
    options.IdleTimeout = TimeSpan.FromMinutes(30);
    options.Cookie.HttpOnly = true;
    options.Cookie.IsEssential = true;
});

var app = builder.Build();

app.UseSession();
```


### 3️⃣ Use Session in CartService

```csharp
public class CartService
{
    private readonly IHttpContextAccessor _httpContextAccessor;

    public CartService(IHttpContextAccessor httpContextAccessor)
    {
        _httpContextAccessor = httpContextAccessor;
    }

    public void AddToCart(int productId)
    {
        var session = _httpContextAccessor.HttpContext.Session;
        var cart = session.GetObject<List<int>>("Cart") ?? new List<int>();
        cart.Add(productId);
        session.SetObject("Cart", cart);
    }
}
```

> 💡 Session data is now stored in Redis — not in the web server.



## 🥈 Option 2: **Database-Driven Cart (Best Design)**

Instead of Session:

```
UserId → Cart → CartItems → Products
```

✔ Works across devices
✔ Survives browser refresh
✔ Ideal for login-based systems

> Amazon, Flipkart, Swiggy → **NOT session based**



## 🥉 Option 3: **Sticky Sessions (❌ Avoid)**

Load balancer forces user to same server.

Problems:

* Not scalable
* Server crash = session loss
* Cloud-unfriendly


## 🧠 Mentor Recommendation (Very Important)

| App Type        | Recommendation              |
| --------------- | --------------------------- |
| Learning / Demo | Distributed Session (Redis) |
| Real E-Commerce | **DB-based Cart**           |
| Microservices   | Token + Cart API            |



## 🔁 Final Decision Flow

```
Single Server?
   |
   No
   |
Need Scalability?
   |
   Yes
   |
Use Distributed Cache (Redis)
   |
OR
   |
Use Database Cart (Best)
```


> **Never store shopping cart session inside the ASP.NET web server in a multi-server setup.
> Store it in Redis or the Database.**





* ASP.NET Core **Web API Controller**
* **Service layer** (Business logic)
* **Repository layer**
* **JSON Helper** for Serialization / Deserialization
* **JSON file** as data store
* **Dependency Injection flow**
* Microservice boundary


## 🧩 ASP.NET Core Microservice – Layered ASCII Architecture

```
┌───────────────────────────────────────────────┐
│                 CLIENTS                       │
│-----------------------------------------------│
│  Browser | Mobile App | SPA | Other Services  │
└───────────────────────────────────────────────┘
                    |
                    | HTTP (JSON Request)
                    v
┌───────────────────────────────────────────────┐
│        ASP.NET CORE WEB API MICROSERVICE       │
│                                               │
│  ┌─────────────────────────────────────────┐  │
│  │            API CONTROLLER               │  │
│  │-----------------------------------------│  │
│  │  ProductsController                     │  │
│  │  - GET /api/products                    │  │
│  │  - POST /api/products                   │  │
│  │                                         │  │
│  │  Depends On → IProductService           │  │
│  └─────────────────────────────────────────┘  │
│                    |                           │
│                    | Dependency Injection     │
│                    v                           │
│  ┌─────────────────────────────────────────┐  │
│  │              SERVICE LAYER               │  │
│  │-----------------------------------------│  │
│  │  ProductService : IProductService        │  │
│  │  - Business Rules                        │  │
│  │  - Validation                            │  │
│  │  - Orchestration                         │  │
│  │                                         │  │
│  │  Depends On → IProductRepository         │  │
│  └─────────────────────────────────────────┘  │
│                    |                           │
│                    | Dependency Injection     │
│                    v                           │
│  ┌─────────────────────────────────────────┐  │
│  │            REPOSITORY LAYER              │  │
│  │-----------------------------------------│  │
│  │  ProductRepository : IProductRepository │  │
│  │  - Data Access Logic                    │  │
│  │  - No Business Rules                    │  │
│  │                                         │  │
│  │  Depends On → IJsonHelper                │  │
│  └─────────────────────────────────────────┘  │
│                    |                           │
│                    | Dependency Injection     │
│                    v                           │
│  ┌─────────────────────────────────────────┐  │
│  │           JSON HELPER UTILITY            │  │
│  │-----------------------------------------│  │
│  │  JsonFileHelper : IJsonHelper            │ │
│  │  - Serialize<T>()                        │ │
│  │  - Deserialize<T>()                      │ │
│  │  - File Read / Write                     │ │
│  └─────────────────────────────────────────┘  │
│                    |                          │
│                    | File I/O                 │
│                    v                          │
│  ┌─────────────────────────────────────────┐  │
│  │              JSON FILE STORE            │  │
│  │-----------------------------------------│  │
│  │  products.json                          │  │
│  │  orders.json                            │  │
│  │  customers.json                         │  │
│  └─────────────────────────────────────────┘  │
│                                               │
└───────────────────────────────────────────────┘
                    |
                    | HTTP (JSON Response)
                    v
┌───────────────────────────────────────────────┐
│                 CLIENTS                       │
└───────────────────────────────────────────────┘
```


## 🔄 Request Flow (Step-by-Step)

```
Client
  ↓
Controller
  ↓
Service
  ↓
Repository
  ↓
JsonHelper
  ↓
JSON File
```

```
JSON File
  ↑
JsonHelper
  ↑
Repository
  ↑
Service
  ↑
Controller
  ↑
Client
```



## 🧠 DI Registration (Mental Model)

```
Controller
   ↓
IProductService  → ProductService
   ↓
IProductRepository → ProductRepository
   ↓
IJsonHelper → JsonFileHelper
```


## 🏗️ Typical Folder Structure

```
ProductMicroservice
│
├── Controllers
│   └── ProductsController.cs
│
├── Services
│   ├── IProductService.cs
│   └── ProductService.cs
│
├── Repositories
│   ├── IProductRepository.cs
│   └── ProductRepository.cs
│
├── Helpers
│   ├── IJsonHelper.cs
│   └── JsonFileHelper.cs
│
├── Models
│   └── Product.cs
│
├── Data
│   └── products.json
│
└── Program.cs
```


> **This JSON-file-based microservice is NOT for production.**
> It is a **teaching microservice** to help students understand:

- ✔ Clean Architecture
- ✔ Dependency Injection
- ✔ Serialization / Deserialization
- ✔ Service–Repository separation
- ✔ Microservice boundaries

Later you replace:

```
JSON File → Database / Cloud Storage
```

**without touching the Controller or Service**



## 🔥 Why This Is a Perfect Teaching Microservice

* No database dependency
* Focus on **architecture**
* Easy to Dockerize
* Easy to convert into:

  * SQL microservice
  * MongoDB microservice
  * Cloud-native microservice

#  **clean, step-by-step way** to create an **ASP.NET Core Web API**.


## 1️⃣ Check .NET SDK

```bash
dotnet --version
```

(Ensure .NET 7 / 8 / 6 is installed)

## 2️⃣ Create Web API Project (Controller-based)

👉 **Important**: Use `--no-minimal` to get **Controllers**

```bash
dotnet new webapi -n ProductMicroservice --no-minimal
```

📁 This creates:

```
ProductMicroservice
├── Controllers
│   └── WeatherForecastController.cs
├── Program.cs
├── ProductMicroservice.csproj
```



## 3️⃣ Go to Project Folder

```bash
cd ProductMicroservice
```


## 4️⃣ Run the Application

```bash
dotnet run
```

Open browser:

```
https://localhost:5001/swagger
```

(or the port shown in console)

---

## 5️⃣ Add Your Own Controller (CLI Way)

Create a **ProductsController**

```bash
dotnet new controller -n ProductsController -o Controllers
```

This creates:

```
Controllers/ProductsController.cs
```


## 6️⃣ Sample ProductsController

```csharp
using Microsoft.AspNetCore.Mvc;

namespace ProductMicroservice.Controllers
{
    [ApiController]
    [Route("api/[controller]")]
    public class ProductsController : ControllerBase
    {
        [HttpGet]
        public IActionResult Get()
        {
            return Ok(new[] { "Laptop", "Mobile", "Tablet" });
        }
    }
}
```

## 7️⃣ Test Endpoint

```
GET https://localhost:5001/api/products
```

Response:

```json
["Laptop","Mobile","Tablet"]
```


## 8️⃣ Program.cs (Controller Enabled)

Make sure you see:

```csharp
builder.Services.AddControllers();

var app = builder.Build();

app.MapControllers();
```


## 9️⃣ Clean Up Default Controller (Optional)

Remove:

```
WeatherForecastController.cs
```


| Goal                 | Command                          |
| -------------------- | -------------------------------- |
| Minimal API          | `dotnet new webapi`              |
| Controller-based API | `dotnet new webapi --no-minimal` |
| Add Controller later | `dotnet new controller`          |

> **Always use controller-based APIs when teaching architecture, DI, and microservices.**





Here is a **clean, simple sample `products.json`** for **Flower products**, perfectly matching your `Product` class.

## 🌸 `products.json`

```json
[
  {
    "Id": 1,
    "Name": "Rose",
    "Price": 25.00
  },
  {
    "Id": 2,
    "Name": "Lily",
    "Price": 40.00
  },
  {
    "Id": 3,
    "Name": "Tulip",
    "Price": 35.50
  },
  {
    "Id": 4,
    "Name": "Orchid",
    "Price": 120.00
  },
  {
    "Id": 5,
    "Name": "Sunflower",
    "Price": 15.00
  }
]
```

## ✅ Matches Your C# Model Exactly

```csharp
public class Product
{
    public int Id { get; set; }
    public string Name { get; set; } = string.Empty;
    public decimal Price { get; set; }
}
```

✔ Property names match
✔ Decimal values supported
✔ Ready for serialization/deserialization

---

## 📁 Recommended Folder Placement

```
ProductWebAPI
├── Data
│   └── products.json
```

 

> JSON files are **excellent for teaching repositories and DI**,
> but **never use them in production** for concurrent systems.



# JsonFileHelper
## **clean, mentor-friendly `JsonFileHelper`** that handles **Serialize & Deserialize** for your **ASP.NET Core Web API microservice**.

This helper is **generic**, reusable, and ideal for your **JSON-file-based repository** teaching model.


## 1️⃣ Interface – `IJsonFileHelper`

📁 **Helpers/IJsonFileHelper.cs**

```csharp
using System.Collections.Generic;

public interface IJsonFileHelper
{
    List<T> ReadFromFile<T>(string filePath);
    void WriteToFile<T>(string filePath, List<T> data);
}
```


## 2️⃣ Implementation – `JsonFileHelper`

📁 **Helpers/JsonFileHelper.cs**

```csharp
using System.Text.Json;

public class JsonFileHelper : IJsonFileHelper
{
    public List<T> ReadFromFile<T>(string filePath)
    {
        if (!File.Exists(filePath))
        {
            return new List<T>();
        }

        var json = File.ReadAllText(filePath);

        if (string.IsNullOrWhiteSpace(json))
        {
            return new List<T>();
        }

        return JsonSerializer.Deserialize<List<T>>(json)
               ?? new List<T>();
    }

    public void WriteToFile<T>(string filePath, List<T> data)
    {
        var options = new JsonSerializerOptions
        {
            WriteIndented = true
        };

        var json = JsonSerializer.Serialize(data, options);
        File.WriteAllText(filePath, json);
    }
}
```

## 3️⃣ Register in Dependency Injection

📁 **Program.cs**

```csharp
builder.Services.AddSingleton<IJsonFileHelper, JsonFileHelper>();
```

## 4️⃣ How Repository Uses This Helper (Conceptual)

```
Repository
   ↓
JsonFileHelper
   ↓
products.json
```

Example usage inside a repository:

```csharp
public class ProductRepository : IProductRepository
{
    private readonly IJsonFileHelper _jsonHelper;
    private readonly string _filePath = "Data/products.json";

    public ProductRepository(IJsonFileHelper jsonHelper)
    {
        _jsonHelper = jsonHelper;
    }

    public List<Product> GetAll()
    {
        return _jsonHelper.ReadFromFile<Product>(_filePath);
    }
}
```

### Why this helper is good for teaching

✔ Separation of concerns
✔ Repository stays clean
✔ File handling logic isolated
✔ Easy to swap JSON → DB later

### Why NOT for production

❌ No concurrency handling
❌ No locking
❌ No transactions


## 📂 Suggested Folder Structure

```
ProductWebAPI
├── Helpers
│   ├── IJsonFileHelper.cs
│   └── JsonFileHelper.cs
├── Repositories
├── Services
├── Controllers
├── Models
├── Data
│   └── products.json
```

> **JsonFileHelper is your “temporary database”**
> Perfect for learning **DI, Repository, Serialization**, and **Microservices boundaries**.

